...
def extract_bulk_info_llm(resume_text, job_description, model=model):
    if not resume_text:
...
def pdfs_to_cleaned_and_extracted_excel(resume_folder, job_description_file, skills_file, final_excel_path, model=model):
    ...
    for filename in tqdm(os.listdir(resume_folder), desc="Processing"):

        # Increment the processed file count
        processed_file_count += 1

        if processed_file_count % 10 == 0:
            # print("\n\n\t\tReinitializing the model after processing 10 files")
        ...
        if filename.endswith(".pdf"):
            ...
            start = time.time()
            extracted_info = extract_bulk_info_llm(resume_text, job_description_text, model=model)
            end = time.time()
